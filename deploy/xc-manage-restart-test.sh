...
MAIN_JAR="xc-manage-exec.jar"

SCRIPTS_DIR=`dirname "$0"`
PROJECT_DIR=`cd $SCRIPTS_DIR && pwd`
DT=`date +"%Y%m%d_%H%M%S"`
STAT_DATE=`date +"%Y%m%d"`
LOG_DIR="/home/logs/xc-manage"

MEM_OPTS="-Xms100m -Xmx500m"
...
nohup java -jar $MEM_OPTS $GC_OPTS $START_OPTS $MAIN_JAR --spring.profiles.active=test > /dev/null 2>&1 &
